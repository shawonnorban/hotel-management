...
class Module extends MX_Controller
{
    ...
    public function install()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $menuPath = APPPATH . 'modules/' . $directory . '/config/menu.php';
            if (file_exists($menuPath)) {
                include($menuPath);

                // Check if the menu is defined
                if (isset($HmvcMenu2[$directory])) {
                    $menuData = $HmvcMenu2[$directory];

                    // Iterate through menu items and insert into language table
                    foreach ($menuData as $key => $item) {
                        if ($key === 'icon') {
                            continue; // Skip the icon key
                        }

                        // Check if the phrase already exists
                        $exists = $this->db->get_where('language', ['phrase' => $key])->row();
                        if (!$exists) {
                            $data = array(
                                'phrase'  => $key,
                                'english' => ucwords(str_replace('_', ' ', $key)), // Convert key to readable text
                            );
            ...
            // Process the database and module creation
            ...
            if (file_exists($dbPath) && file_exists($configPath)) {
                ...
                if (($HmvcConfig[$directory]["_database"] === true) && !empty($HmvcConfig[$directory]["_tables"])) {
                        ...
                        if (!empty($equery_string)) {
                            ...
                            foreach ($equery_list as $equery_item) {
                                if (!empty($equery_item)) {
                                    // Check if the column 'device_token' already exists
                                    if (strpos($equery_item, 'ALTER TABLE `user` ADD `device_token`') !== false) {
                                        // Check if column exists
                                        $query = $this->db->query("SHOW COLUMNS FROM `user` LIKE 'device_token'");
                                        $column_exists = $query->num_rows() > 0;

                                        if (!$column_exists) {
                                            // Column doesn't exist, proceed to add it
                                            $this->db->query("ALTER TABLE `user` ADD `device_token` TEXT NULL DEFAULT NULL AFTER `email`");
                                        }
                                    }
                                    // Check if the column 'waiter_kitchenToken' already exists
                                    elseif (strpos($equery_item, 'ALTER TABLE `user` ADD `waiter_kitchenToken`') !== false) {
                                        // Check if column exists
                                        $query = $this->db->query("SHOW COLUMNS FROM `user` LIKE 'waiter_kitchenToken'");
                                        $column_exists = $query->num_rows() > 0;

                                        if (!$column_exists) {
                                            // Column doesn't exist, proceed to add it
                                            $this->db->query("ALTER TABLE `user` ADD `waiter_kitchenToken` TEXT NULL DEFAULT NULL AFTER `device_token`");
                                        }
                                    } else {
                                        // Run the query if it's not related to adding specific columns
    ...
    public function uninstall($dirPath = null, $action = null)
    {
        ...
        if (is_dir($dirPath) && $dirPath != $basePath) {
            ...
            if (file_exists(APPPATH . 'modules/' . $directory . '/assets/data/env') && $HmvcConfig[$directory]["_extra_query"] === true) {
            ...
            // Remove language phrases
            if (file_exists($menuPath)) {
                @include($menuPath);

                if (!empty($HmvcMenu2[$directory])) {
                    foreach ($HmvcMenu2[$directory] as $menuKey => $menuItem) {
                        // Remove the key from the language table
                        $this->db->where('phrase', $menuKey)->delete('language');
                    }
                }
            }
            ...
            if ($action == "delete") {
                ...
                $this->db->where('directory', $directory)->delete('module');
            }
    ...
    private function create_module_json()
    {
        ...
        if (isset($HmvcMenu) && $HmvcMenu != null && sizeof($HmvcMenu) > 0) {
            ...
            $cache_file = './assets/js/addon.json';
    ...
    public function dowunloadzip()
    {
        $this->load->helper('download');
        force_download('application/modules/hrmscheduler/assets/hrmscheduler/emp_attendance_app.zip', NULL);
    }
    ...
    {
        $pkey = $this->db->select("purchasekey")->from("tbl_module_purchasekey")->where("module", "house_keeping")->order_by("mpid", "DESC")->get()->row();
        $domain = base_url();
        $validate = $this->addons_model->send_app_download_request($domain, $pkey->purchasekey);
