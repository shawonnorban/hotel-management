...
class Addons_model extends CI_Model
{
    ...
    private $app_url = "https://store.bdtask.com";
    private $header;
    ...
    // Send New app redirect download request
    public function send_app_download_request($domain, $purchase_key)
    {
        $url = $this->app_url . "/class.addon.php?domain=" . $domain . "&product_key=" . $this->product_key . "&purchase_key=" . $purchase_key;
        $return = $this->send_curl_request($url);
        return json_decode($return);
    }
}
