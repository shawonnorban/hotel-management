...
public function create_shortlist()
	{ 
		$this->form_validation->set_rules('can_id',display('can_id'),'required|is_unique[candidate_shortlist.can_id]|max_length[50]|xss_clean');
		$this->form_validation->set_rules('job_adv_id',display('job_adv_id')  ,'required|max_length[100]|xss_clean');
		$this->form_validation->set_rules('date_of_shortlist',display('date_of_shortlist'),'required|max_length[50]|xss_clean');
		$this->form_validation->set_rules('interview_date',display('interview_date')  ,'required|max_length[100]|xss_clean');
		if ($this->form_validation->run() === true) {
			...
			redirect("hrm/candidate-shortlist");

		} else {
			$data['title'] = display('candidate_shortlist');
			$data['module'] = "hrm";
	...
	{ 
		...
		redirect('hrm/manage-shortlist');
	}

public function update_shortlist_form($id = null){
 		$this->form_validation->set_rules('can_short_id',null,'required|max_length[11]|xss_clean');
 		$this->form_validation->set_rules('can_id',display('can_id'),'required|max_length[50]|xss_clean');
		$this->form_validation->set_rules('job_adv_id',display('job_adv_id')  ,'required|max_length[100]|xss_clean');
		$this->form_validation->set_rules('date_of_shortlist',display('date_of_shortlist'),'required|max_length[50]|xss_clean');
		$this->form_validation->set_rules('interview_date',display('interview_date')  ,'required|max_length[100]|xss_clean');
		if ($this->form_validation->run() === true) {
			...
			redirect("hrm/shortlist-update/". $id);
...
public function create_interview()
	{ 
		...
		$this->form_validation->set_rules('can_id',display('can_id'),'required|is_unique[candidate_interview.can_id]|max_length[50]|xss_clean');
		$this->form_validation->set_rules('job_adv_id',display('job_adv_id')  ,'required|max_length[100]|xss_clean');
		$this->form_validation->set_rules('interview_date',display('interview_date')  ,'required|max_length[100]|xss_clean');

		$this->form_validation->set_rules('interviewer_id',display('interviewer_id'),'required|max_length[50]|xss_clean');
		$this->form_validation->set_rules('interview_marks',display('interview_marks')  ,'required|max_length[100]|xss_clean');
		$this->form_validation->set_rules('written_total_marks',display('written_total_marks')  ,'required|max_length[100]|xss_clean');
		$this->form_validation->set_rules('mcq_total_marks',display('mcq_total_marks'),'required|max_length[50]|xss_clean');
		$this->form_validation->set_rules('total_marks',display('total_marks'),'required|max_length[50]|xss_clean');
		$this->form_validation->set_rules('recommandation',display('recommandation')  ,'max_length[100]|xss_clean');
		$this->form_validation->set_rules('selection',display('selection')  ,'required|max_length[30]|xss_clean');
		$this->form_validation->set_rules('details',display('details')  ,'max_length[100]|xss_clean');
		if ($this->form_validation->run() === true) {
  ...
  public function delete_interview($id = null){ 
     $this->permission->module('hrm','delete')->redirect();
		...
		redirect("hrm/manage-interview");
	}

	public function interview_update_form($id = null){
		$this->form_validation->set_rules('can_int_id',null,'required|max_length[11]|xss_clean');
 		$this->form_validation->set_rules('can_id',display('can_id'),'required|max_length[50]|xss_clean');
		$this->form_validation->set_rules('job_adv_id',display('job_adv_id')  ,'required|max_length[100]|xss_clean');
		$this->form_validation->set_rules('interview_date',display('interview_date')  ,'required|max_length[100]|xss_clean');
		$this->form_validation->set_rules('interviewer_id',display('interviewer_id'),'required|max_length[50]|xss_clean');
		...
		$this->form_validation->set_rules('written_total_marks',display('written_total_marks')  ,'required|max_length[100]|xss_clean');
		$this->form_validation->set_rules('mcq_total_marks',display('mcq_total_marks'),'required|max_length[50]|xss_clean');
		$this->form_validation->set_rules('recommandation',display('recommandation')  ,'required|max_length[100]|xss_clean');
		$this->form_validation->set_rules('selection',display('selection')  ,'required|max_length[30]|xss_clean');
		$this->form_validation->set_rules('details',display('details')  ,'required|max_length[100]|xss_clean');
		if ($this->form_validation->run() === true) {
			...
			if ($this->Selection_model->update_interview($postData)) { 
				...
				$this->session->set_flashdata('exception',  display('please_try_again'));
				redirect("hrm/interview-update/". $id);
			}	
...
public function create_selection()
	{ 
		...
		$this->form_validation->set_rules('can_id',display('can_id'),'required|is_unique[candidate_selection.can_id]|max_length[50]|xss_clean');
		//  $this->form_validation->set_rules('pos_id',display('pos_id'),'required|max_length[50]|xss_clean');
		$this->form_validation->set_rules('selection_terms',display('selection_terms')  ,'required|max_length[100]|xss_clean');
		...
		if ($this->form_validation->run() === true) 
		{
			// dd($this->input->post('can_id',TRUE));
			$postData =array(
				'can_id' 	     => $this->input->post('can_id',TRUE),
           ...
           if($this->Selection_model->selection_create($postData))
		   {
			...
			$this->session->set_flashdata('message', display('successfully_saved'));
			redirect("hrm/candidate-selection");
			}
			else
			{
				$this->session->set_flashdata('exception', display('please_try_again'));
	...
	{ 
		...
		redirect("hrm/manage-selection");
	}

       public function update_selection_form($id = null){
 		$this->form_validation->set_rules('can_id',display('can_id'),'required|max_length[50]|xss_clean');
		$this->form_validation->set_rules('pos_id',display('pos_id')  ,'required|max_length[100]|xss_clean');
		$this->form_validation->set_rules('selection_terms',display('selection_terms'),'max_length[100]|xss_clean');
		$employee = $this->db->select('*')->from('candidate_basic_info')->where('can_id',$id)->get()->row();
		...
		if ($this->form_validation->run() === true) {
			...
			if ($this->Selection_model->update_selection($postData)) { 
			  ...
			  ->update("employee_history", $Data1);
				...
				$this->session->set_flashdata('exception',  display('please_try_again'));
				redirect("hrm/manage-selection/". $id);
			}
