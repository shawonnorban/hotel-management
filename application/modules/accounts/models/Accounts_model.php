...
public function voNO()
 ...
 // Contra voucher 
  ...
  public function insert_creditvoucher() {
    $voucher_no = addslashes(trim($this->input->post('txtVNo', TRUE)));
    $Vtype = "CV";
    $dAID = $this->input->post('cmbDebit', TRUE); // Single value
    ...
    // Validate inputs
    if (!is_array($cAID) || !is_array($Credit)) {
        echo '<pre>';
        echo 'txtCode: ';
        print_r($cAID);
        echo 'cmbDebit: ';
        ...
        print_r($Credit);
        echo '</pre>';
        throw new Exception('Invalid input data for txtCode or txtAmount.');
    }
    ...
    // Insert into `acc_income_expence`
    for ($i = 0; $i < count($cAID); $i++) {
        $crtid = $cAID[$i];
        $Cramnt = $Credit[$i];
        $debitinsert = [
...
//approved
    ...
    public function update_creditvoucher() {
        $voucher_no = addslashes(trim($this->input->post('txtVNo', TRUE)));
        $Vtype = "CV";
        $dAID = $this->input->post('cmbDebit', TRUE); // Single value or array
        $cAID = $this->input->post('txtCode', TRUE);
        $Credit = $this->input->post('txtAmount', TRUE);
        $debit = $this->input->post('grand_total', TRUE);
        $VDate = $this->input->post('dtpDate', TRUE);
        ...
        $this->db->where('VNo', $voucher_no)->delete('acc_transaction');
